...
- 生产批次先按物流和订单组成分组。单项单件再分单面与双面；双面不分颜色、底款或尺码。
  单面分黑色、白色和其他颜色混色，不按底款拆分；黑白继续分`S/M/L/XL`与
  `XXL(2XL)/3XL/4XL/5XL`两档，其他颜色不分尺码。单项多件和多项多件不再拆分。
...
- 防重顺序：固定状态`9`；读取实际面别；保留来源批次号；按整单分组；提交前重新读取状态、
  ...
  任何变化均停止。平台可能把一个分组拆成多个编号，写后逐订单确认每单只对应一个新增批次，
  并返回本次全部编号。API结果不确定时不得自动重试，先查批次页和项目补单明细。
...
开发者模式 → 生产平台下载 → Haloo → 已完成订单计划。页面先完整显示已确认的Haloo默认规则，并允许用户自由组合订单组成、物流、单双面、颜色、尺码档和底款六项；选择按平台保存，修改后旧预览立即清空。默认读取30项，可选择1–200项；读取由后台任务执行，显示实际采用的分组字段、来源批次、生产项ID及未纳入数量。“自动化生成计划”读取相同范围并自动勾选尚未补单的候选分组，不提交批次；也可手动读取分类和勾选。勾选分组时，下方“待生成清单”立即逐组展示实际组合及件数；确认窗口再次完整列出，确认后才提交。按每组单独生成补单批次；已有补单的组不可勾选。改变样本范围或分组组合会清空旧结果。

隆丰和莆田现与Haloo共用“已完成订单计划”页签和后台规划入口，分别读取各自平台并采用各自分组策略；本文的接口实证仅覆盖Haloo。
