...
- Supabase机器状态与远程任务中心由`202609220001/2`状态迁移及`202609230001/2`指令迁移维护；表仅service role可访问，Edge Function通过受限工厂密钥提供状态和短期指令接口。`automation/api/printerexp/`每2秒从正在运行的PrintExp进程反查真实安装目录，读取保留小数的实际进度并按变化或60秒心跳上报，小数变化直接参与剩余时间估算；显示名只接受同一台AutomaticPrint排版设置中的M1–M11机器号，旧环境变量、电脑名和其他备注不得覆盖。主工作台固定显示M1到M11并每10秒后台刷新，忽略旧名称记录；同一机器号存在多个在线电脑时合并为一个冲突机位并禁止远程控制，避免控制错机。打印机控制提供两个用户手动按钮：“暂停打印”只切换到已暂停；“清洗后自动启动”会先确认当前正在打印并自动暂停，确认暂停后才启动清洗，等待清洗状态结束后再恢复打印，任一步状态不明或15分钟超时均不继续下发下一步并回传原因。主程序每次启动都会自修复当前用户的监控启动项并启动受单实例锁保护的后台代理。控制端仍可向监控在线的目标机下达下载排版任务，目标机后台原子领取后启动隔离任务进程并复用现有生产流程；过程和结果统一回传中心。
