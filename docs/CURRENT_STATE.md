...
- 普通模式显示生产排版规则、45/60厘米方案、批次记录入口和额外损耗；本地排版平台列表包含Haloo，莆田仍须开发者模式。生产平台下载工作区仍仅开发者可见。所有切膜机参数集中显示，包括刀码、刀位、膜标签补距、换刀停止距离、识别色块和切膜提示；旋转、并排、缩放、间距与膜方案比较统一归入自动排版。切换到正常排版（无刀码）时隐藏补距，不向普通打印模式展示无效参数；“允许旋转”仍可用，并在一次有序求解中比较原方向、局部旋转和整批旋转，切膜快速模式不再误关该能力。补足膜间距
