...
- “生产平台下载”作为跨部门共享页签；UV 工作区直接显示，DTF 工作区仍随开发者模式显示。支持多选已配置平台，每个平台独立显示批次、下载进度和日志。普通下载不触发排版；蜂鸟平台的显式“下载、排版并生成打印文件”复用当前参数继续处理。隆丰、莆田和Haloo复用蜂鸟ERP通路；S2B优先通过Supabase受限网关读取平台批次、人员标签及标准化图片数/件数，用户选择后自动触发缺失的生产图导出、轮询、下载并安全解压到`S2B/ARCHIVES`和`S2B/BATCHES`，成功解压后才标记下载；原始S2B Token只在服务端解密，网关不可用时才回退专用浏览器登录。浏览器登录成功后会把Token交给同一受限网关验证，并在服务端AES-GCM加密更新`S2B:DTF`凭据，客户端日志和配置不保存原始Token。S2B工作区另有“分批规则”页，默认关闭物流并按平台保存全部组合。站点的建批接口已确认是`POST /req/factory/orderProductBatchNumber`，请求包含计划名称、人员、超期小时、备注、生产项ID集合和是否补单；应用尚未调用该写接口，须先读取当天未组批订单并完成预览与写后复核。亿点万象只下载具体勾选批次中的新增稿件，与UV排版分开。下载和自动打印动作都不自动创建生产批次。
