...
- 打印机状态页提供每台机器都可使用的“打印历史”子页。选择M1–M11后只在用户点击时通知目标节点，目标节点直接读取自己的PrintExp每日主日志，以“启动任务/作业打印完成”配对取得可靠开始、结束时间，并用`Data/recordTask.tf`补充原始路径；默认返回本机日期的今天和昨天。若目标机没有对应日期日志，则明确标注日期降级并显示最近两个可用日志；若日志没有时间事件，则显示`recordTask.tf`中的本地任务记录并明确标记开始、结束时间不可用，不能误报没有历史或伪造时间。历史不周期上传、不复制到状态表，也不建立云端历史库。跨机器返回仅使用现有命令信封暂存，请求端读到后立即删除整条临时命令；本机查询完全不经过Supabase。
...
- 打印机状态页的“版本管理”按M1–M11独立显示当前版本、所选目标版本及等待、领取、切换、失败、源码完成待重启和最终版本回报，本机按UUID明确标为“（本机）”。版本列表在后台从`origin/main`读取；用户可选择已发布且不早于0.1.393的历史版本，全选待切换机器、取消全选或逐机勾选，并在下发前查看每台当前→目标版本及完整提交号。升级与回滚复用同一`source_update`命令和UDP优先、Realtime后备的按需唤醒；目标机重新核对官方仓库、main线性历史及提交中的版本元数据，有已跟踪本地修改时拒绝覆盖。向前只快进，回滚使用保留本地内容的`reset --keep`且不脱离main，因此回滚后仍可再次升级；依赖完成后先回传结果，再由脱离监控任务Job的隐藏辅助进程结束旧监控并重新运行计划任务，避免辅助进程随旧任务一同被终止。只有存在活动切换时每5秒读取回执，全部结束后停止。监控启动时只补领一次普通命令；AutomaticPrint有生产任务时安全重启守卫等待空闲，不强制停止。早于支持`source_update`的旧版电脑仍需最后手动更新一次。
