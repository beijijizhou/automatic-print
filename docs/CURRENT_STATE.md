...
- 蜂鸟ERP已完成候选批次由`automation/batches/supplements/completed.py`共用安全核验，由`strategies.py`按平台决定分组字段。已有批次的完成订单走补单批次接口，不改变订单完成状态；提交前通过`order_id`精确读取整单并重新核对状态、来源批次、数量和项目集合。普通精确选择接口会排除已有批次项目。平台把一次提交拆成多个批次号时，程序逐订单确认每单只归属一个新增批次并返回全部编号。Haloo和隆丰规则已用历史真实批次回放验证；莆田仍需真实平台回归。
...
- 生产平台的目标机器选择由`batch_ui/platform/remote_machine_dialog.py`与`remote_queue.py`合并状态心跳和
