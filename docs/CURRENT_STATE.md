...
- 普通模式显示生产排版规则、45/60厘米方案、批次记录入口和额外损耗；本地排版平台列表包含Haloo，莆田仍须开发者模式。生产平台下载工作区仍仅开发者可见。膜标签补距属于切膜机专用参数，主界面与完整参数窗口都和刀码、刀位、旋转区、并排缩小集中展示；切换到正常排版（无刀码）时隐藏补距，不向普通打印模式展示无效参数。补足膜间距
...
- 主界面“切膜机专用”参数组集中提供切膜刀码、膜标签补距、并排缩小和平台尺码标签开关；开发者模式再显示批次顺序和方案比较。切膜刀码开关在正常排版与上次
...
- `ui/batch_input_panel.py` 拥有主界面的批次操作和常用参数分组；最上方蓝色批次卡片先显示当前目录，
  ...
  使用同一层级；完整参数窗口也由“切膜机专用”页唯一拥有所有刀码相关控件和补距，底部恢复/保存按钮固定在滚动内容之外。主界面提供持久化的“打开最近生成的批次”
