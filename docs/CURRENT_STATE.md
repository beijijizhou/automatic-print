...
- 后台机器号使用独立的`machine-binding`文件并保持原机器UUID。新版本只有用户明确选择或保存M1–M11时才更新该绑定；旧版本窗口、普通排版偏好、注册表和兼容`machine-name`文件即使再次写入其他号码，也不能覆盖后台监控与远程命令使用的机器身份。
...
- 发送生产任务不再依据数据库中的旧在线状态直接放行。用户选择目标机后先建立一次性探测并通过签名局域网通知指定机器领取；目标机必须在12秒内以同一机器ID和M1–M11机器号回传AutomaticPrint版本及现场读取的PrintExp状态。只有PrintExp在线且回应完全匹配才显示最终发送确认；发送流程没有自动化开关步骤，超时、机器号不一致或PrintExp离线仍不会创建生产任务。探测记录不进入任务队列展示，也不启动任何周期轮询。
