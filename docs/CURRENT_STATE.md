...
- Supabase机器状态与远程任务中心由`202609220001/2`状态迁移及`202609230001`指令迁移维护；表仅service role可访问，Edge Function通过受限工厂密钥提供状态和短期指令接口。`automation/api/printerexp/`每2秒从正在运行的PrintExp进程反查真实安装目录，读取保留小数的实际进度并按变化或60秒心跳上报，小数变化直接参与剩余时间估算；显示名读取同一台AutomaticPrint排版设置中的M1–M11机器号。主工作台按M1到M11固定显示11个机位并每10秒后台刷新。主程序每次启动都会自修复当前用户的监控启动项并启动受单实例锁保护的后台代理，因此只做源码更新的旧电脑无需重新运行安装脚本。控制端可向监控在线的目标机下达最多20个12位批次号的下载排版任务，目标机后台原子领取后启动隔离任务进程，使用目标机自己的ERP登录、下载目录和机器编号，复用现有批次类型核对、下载、排版及RIIN流程；任务30分钟未领取自动过期，过程和结果回传中心。远程流程只生成PNG或生成PRN并加载PrintExp，不启动物理打印。
