...
- 打印机状态页的“控制与任务”提供一键更新全部已登记源码电脑。控制端仅允许把当前已推送到`origin/main`的精确Git提交和版本分别写入每台机器的`source_update`命令，并复用签名UDP立即唤醒；目标机沿用`SourceUpdater`的官方仓库、main分支、无已跟踪本地修改和只快进保护，更新依赖后先回传结果，再重启后台监控。M1–M11表独立显示当前版本及等待、领取、更新、失败、源码完成待重启和最终版本回报，只有存在活动更新时每5秒读取回执，全部结束后停止。监控启动时只补领一次普通命令，覆盖电脑关机时错过UDP的更新，不恢复周期轮询；AutomaticPrint有生产任务时仍由安全重启守卫等待空闲，不强制停止。旧版电脑需要最后手动更新到支持`source_update`的版本一次。
