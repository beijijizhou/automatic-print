...
- 打印机状态页提供“一键开启全部自动化／一键关闭全部自动化”。关闭信号通过带工厂密钥HMAC签名的局域网UDP广播发送给M1–M11；各后台监控持久进入零云流量状态，停止Supabase心跳、下载任务领取和打印机控制轮询，状态页也停止自动刷新，但电脑、AutomaticPrint、PrintExp和本地UDP监听保持运行。开启同样通过局域网签名广播立即恢复，无需停用期间轮询Supabase；安装脚本为专用UDP端口创建仅限Domain/Private与LocalSubnet的入站防火墙规则。
- 云调用安全门以“未明确开启即关闭”为默认，新安装和更新后不会自动连接Supabase。用户开启后，状态只在任务、状态或整数进度变化及300秒心跳时上报，ETA连续变化不触发请求；后端离线阈值为420秒，打印机状态页开启时每60秒刷新。高频命令轮询仅保留在用户明确开启的兼容路径，重新部署`machine-status`前需迁移到Realtime或等价的服务端推送。
