...
- 隆丰 A00 已接单有图多项多件订单由`automation/batches/received_sizes.py`读取接口快照并规划：跨尺码整单优先，同尺码再按尺码分别生成；单项单件及单项多件不纳入。复用`automation/api/erp/items.py`的整单查询及精确选择生成接口，每次不超过200生产项且只在订单边界分段；逐单确认入批次号，单组不确定不重试，其余独立组继续。当前通过开发操作调用。
- 已入生产中的 A00 有图多项多件由`automation/batches/production_multi.py`限定来源批次，用补单接口按跨尺码及八个同尺码组重组；一次提交的跨尺码394项被平台拆成200项和194项两批。逐单核对同批、补单数量及批次管理合计；无安全合并能力时保留实际两批，禁止再次补单。
