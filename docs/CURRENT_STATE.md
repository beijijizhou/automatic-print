...
- 生产平台下载位于部门工作区之外的共享页签，继续由 `ui/erp_download_entry.py` 装配同一个
  `ProductionPlatformDownloadPage`；DTF 与 UV 普通用户均可直接使用，不再受开发者模式门禁。普通下载不自动排版；
...
- 顶部开发者菜单显示“完整测试”按钮及实时结果；完整自动测试逐项显示测试文件、名称和通过/失败状态，状态栏同步显示当前项目，不再只显示圆点；随后运行本机Haloo、隆丰、莆田、S2B真实批次回归，只生成测试PNG及报告，不生成、装载或发送PRN。开发者导航将标签位置短测、随机10批冷启动、批量膜分析与算法诊断集中在一个
...
- 普通模式显示生产排版规则、45/60厘米方案、批次记录入口和额外损耗；本地排版平台列表包含Haloo，莆田仍须开发者模式。生产平台下载工作区对DTF和UV普通用户开放。所有切膜机参数集中显示，包括刀码、刀位、膜标签补距、换刀停止距离、识别色块和切膜提示；旋转、并排、缩放、间距与膜方案比较统一归入自动排版。切换到正常排版（无刀码）时隐藏补距，不向普通打印模式展示无效参数；“允许旋转”仍可用，并在一次有序求解中比较原方向、局部旋转和整批旋转，切膜快速模式不再误关该能力。补足膜间距
  由 `ui/header_gap.py` 的独立开关控制，保存的毫米数值本身不会自动启用；`layout_engine/labeling/base/header_gap.py`负责批次编排，`layout_engine/labeling/gap/virtual.py`让Haloo、S2B、莆田和隆丰持久缓存间距几何并在最终合成时插入透明像素，不创建中间大图；其他平台仍由`layout_engine/labeling/gap/preparation.py`生成兼容副本，`layout_engine/labeling/gap/cache/cache_files.py`负责Windows占用重试和临时文件回收。补距和超宽缩小缓存使用文件指纹的96位目录摘要及短临时后缀，为长生产文件名保留Windows路径余量，身份核验仍使用完整指纹。补距会越过Haloo标签不属于白色卡片连通域的彩色底栏，再从真实透明分界补足40毫米。批次预览和最终报告显示总数、实际扩充、原本已满足、未能扩充及新增毫米范围。开发者模式显示算法
...
- 主窗口在所有工作区 Tab 上方固定显示全局生产排版模式，完整设置窗口也在所有参数 Tab 与滚动内容上方固定显示同一控件；两处直接镜像唯一的切膜模式状态，切膜时用橙色粗边框警示，正常排版时用绿色明确标注“无刀码”。主界面明确分为“切膜机 · 刀码与补距”和“自动排版”；切膜机区不再重复显示第二个刀码总开关，只保留补距独立开关，并直接显示膜规格、模式、有效画布、刀位方式与数值、左刀码高度、换刀/批次结束停止距离和识别色块入口。完整设置窗口的“切膜机”页集中所有切膜机相关参数，“自动排版”页集中旋转、并排、缩放、间距与膜方案比较。换刀停止距离是普通切膜生产参数，切膜模式下直接生效。开发者模式再显示批次顺序和方案比较。切膜刀码开关在正常排版与上次
...
- `ui/batch_input_panel.py` 拥有主界面的批次操作和常用参数分组；最上方蓝色批次卡片先显示当前目录，
  ...
  同步切换当前子批次，不重新读取图片或刀码。膜规格和排版方式与它并列，批次、输出和切膜机专用参数
  使用同一层级；完整参数窗口将切膜机参数集中在同一页，自动排版参数集中在另一页，底部恢复/保存按钮固定在滚动内容之外。主界面提供持久化的“打开最近生成的批次”
  快捷入口；批次顺序标注仍受开发者模式控制。
  `ui/workbench/overview/`按界面区域分别拥有快捷标签、批次数据、预览和信号联动；进入时默认展示
  可复制的文字排版预览；已保存的批次路径在启动后后台读取文件名，不启动排版。文件名清单先显示多件“订单号-件数-尺码”，单件与双面按尺码合并计数并明确标注不是排版位置，
  不再以逐件竖排清单冒充摆放顺序；排版确定后按真实坐标标注单排或双排，内部“并排区”对生产人员统一显示为“常规区”，与“旋转区”并列；双排使用不换行的固定宽度文字逐排显示左侧、分割线、右侧，
  ...
  框下按实际标签内容以更大字号显示可复制的放大文字；页面顶部还从同一张已渲染生产示例中截取
  局部放大优先直接从当前批次真实原图取样，无当前批次时使用仓库内置的真实 Haloo 样本；同一张大预览保留刀码、膜标签和文字标签的真实坐标及比例，分别用红、橙、紫框标识。文字原位由紫色连线指向同图下方的放大镜，放大文字在预览 DPI 下用同一内容、字体和换行规则清晰重绘，不改变原坐标。顶部同时显示可复制的大字原文，可切换左右与旋转情况。
...
- 生产平台批次页只保留逐行选择、刷新、“下载、排版并生成打印文件”和“发送到指定机器生成 PRN”；
  不再显示“全选可下载批次”“下载并解压”“下载完成后打开文件夹”“多批次共用刀位生成PRN”及
  已下载批次重新排版入口。共享刀位工作流源码仍保留供历史兼容，但当前没有生产平台可见入口。
...
- S2B 文件夹批次号解析、中心批次查询及本地图片颜色匹配位于
  `automation/api/s2b/metadata/`，生产批次列表和导出下载位于`production/`；连字符和下划线文件名均提供批次号、订单项及尺码，文件名尺码优先于可能失准的子目录名。日期与批次号不完整的 S2B 文件夹从图片恢复批次号；同目录混合导出的图片按实际批次分别查询，并按订单项和尺码匹配颜色，双面图片按同商品成对归组。只要识别到 S2B 批次，预览和生成都会在排版前查询订单颜色并按本地
...
- 行、刀位和区域规划：`layout_engine/planning/base/planner.py`、`layout_engine/planning/base/row_optimizer.py`、`layout_engine/planning/columns/cutter_planner.py`、`layout_engine/planning/columns/dynamic_columns.py`、
  ...
  独立工作树正在验证引擎级`counting_accuracy_layout`试验策略；它没有接入主界面，也不改变默认排版。
  该策略仅保留每排恰好两张且均不旋转的并排行，其余完整订单进入后续单排区。坐标稳定后每排先编左图、再编右图，然后进入下一排。
...
- 主窗口在全部工作区 Tab 上方固定显示唯一的“统一任务进度”，集中展示当前任务、步骤、对象、
  真实进度、步骤/总耗时和停止入口；DTF、生产平台、UV、亿点万象与源码更新都写入同一状态中心，
  页面内不再重复展示各自的进行中状态。M1–M11 打印机状态目前只在“打印机状态”Tab 展示，
  该页默认优先显示完整 11 机表格，控制和远程任务位于同页的第二个子页。
...
- 自动化排版参数由`history/layout_settings.py`保存和读取执行电脑的完整本地快照；目标机执行远程批次时，刀码模式及其他生产参数不由发送机覆盖。
  无快照的旧安装从本机`cutter/mode`及配套切膜参数兼容恢复；快照损坏时也使用该回退。共刀、整单归侧和批次序号等单次任务值不写入本地默认快照。
- 主界面顶部只常显版本、手动“检查更新”和开发者模式开关；开启开发者模式后，一个开发者功能区集中显示打印参数、DTF平台账号、诊断与完整测试，不再显示重复的功能清单或第二套入口。“当前生产信息”显示机器、平台、膜规格、有效画布及同一个本地`cutter.mode`。生产模式是用户唯一可见的模式入口，批次膜规格卡片与切膜参数页不再重复显示模式下拉框。手动与启动检查复用更新模块，软件检查、下载、应用和重启不写入只用于生产步骤的任务中心。
- 主工作台“预览检查”在没有批次内容时限制为96像素高的空状态提示；批次读取或排版开始后恢复页签但总高度限制为560像素，内部预览滚动区限制为460像素，放大检查仍使用独立最大化窗口，避免遮挡固定在滚动区外的开发者入口。只有路径而没有内容时仍保持紧凑，清空路径时再次收起。
- Supabase机器状态与远程任务中心由状态、机器注册、指令和事件可用性迁移维护；表仅service role可访问，Edge Function通过受限工厂密钥提供注册、状态、下载任务和实时控制信号接口。`automation/api/printerexp/`每2秒在本机读取PrintExp，只有任务、打印机状态或整数进度变化才覆盖当前行；发送端建立请求后先用HMAC签名UDP通知唯一机器ID并等待目标机签名ACK，仅在ACK超时时用Supabase Realtime跨网络通知，不做周期心跳、命令轮询或界面定时刷新。后台机器号以数据库中机器UUID的一对一注册为准，本地`machine-slot-v2`仅缓存成功注册结果；普通参数、旧环境变量、电脑名、注册表和旧身份文件均不能更新云端身份。主工作台固定显示M1到M11，未注册电脑不能上报状态。生产批次页把勾选批次与排版参数发给实时探测通过的机器，目标机连续执行下载、排版、生成PRN和加载PrintExp，不启动物理打印。暂停和清洗继续使用独立高优先级控制通道；清洗逻辑保留现状，后续单独完成现场验证后再修改。源码安装/更新脚本注册与PrintExp同权限的登录计划任务，更新时重启监控。过程和结果统一回传中心。
- 打印机状态页提供每台机器都可使用的“打印历史”子页。选择M1–M11后只在用户点击时通知目标节点，目标节点与机器状态使用同一个正在运行的PrintExp安装目录，并读取其每日主日志，以“启动任务/作业打印完成”配对取得可靠开始、结束时间；从四种原生任务文件中选择实际有记录的数据源补充路径。默认返回本机日期的今天和昨天；若没有对应日期日志，则明确标注日期降级并显示最近两个可用日志；若日志没有时间事件，则显示本地任务记录并明确标记开始、结束时间不可用，不能误报没有历史或伪造时间。历史不周期上传、不复制到状态表，也不建立云端历史库。跨机器返回仅使用现有命令信封暂存，请求端读到后立即删除整条临时命令；本机查询完全不经过Supabase。

- PrintExp实时控制增加`202609230004_start_print_control.sql`、`202609230005_start_print_action.sql`和“开始打印”。`automation/api/printerexp/state_machine.py`统一拥有空闲、待打印、打印中、已暂停、清洗中和未确认六种原始状态，以及平台投影、显示名称、控制许可和转换路径；控件识别、状态上报、目标机执行器和UI只消费该规则。Supabase入口对同一契约做边界校验，拒绝平台状态与PrintExp状态互相矛盾的上报及控制请求；数据库只存当前观察事实。同一按钮在0%待打印时开始新任务，在已暂停时显示“继续打印”并恢复当前任务。两种状态都必须已装载对应批次且任务名已独立核实。服务端先核对当前状态与批次名，目标机执行前再读取状态文件并核对PrintExp界面中的实际PRN名称，任一事实变化均拒绝发送指令。
- 远程下载排版与打印控制在执行前后写入本机`command-journal.sqlite3`，把Supabase `command_id`关联到动作、批次/PRN、PrintExp任务ID、物理打印阶段、完整结果和云端回执状态。相同终态指令再次到达时只重放已保存回执，不再次下载、生成或点击打印；发现同一指令已有非终态本地记录时停止自动重试，保留“等待人工核查”事实。开始打印返回PrintExp `TASK_GUID`并先落本地，后台按任务ID和PRN名共同核对完成状态；同名PRN的不同任务ID不得合并。云端回执失败时本地记录保持未上报，监控启动或状态变化时补交，不以重做物理动作代替回执恢复。
- PrintExp文件打开对话框提交路径并确认关闭后，会把本次PRN写入本机未核实装载回执；当`PrintInfo.ini`仍停留在上一任务时，监控优先显示该新PRN为0%“待打印（待复核）”。只有界面或状态文件独立确认文件名后才允许“开始打印”，装载回执本身不能触发物理打印。远程PRN结果包含生成或装载错误时，整条命令标记失败并保留详细结果，不再显示成功。
- 打印参数“输出与并行”提供遮罩的工厂服务密钥入口。用户显式保存后写入当前 Windows 用户的唯一网关密钥缓存，界面不回显已有值，也不写入普通偏好设置或日志；后台监控后续连接直接复用该缓存，不再要求该电脑必须访问共享盘。
- 打印机状态页提供“测试机器信号”按钮，并发探测已登记且无编号冲突的M1–M11，分别显示本次实时响应、上次上报版本、当前版本数量、待更新和超时机器；检测结果留在状态页，不进入生产任务中心。
- M1–M11由Supabase `machine_registry`按机器UUID唯一绑定；状态上报忽略客户端提交的名字并使用数据库注册值。打印机状态页提供“本机注册”，首次注册和占用换绑都须人工确认，数据库事务锁避免并发双绑定；换绑后删除旧UUID同机位状态。`machine-slot-v2`只缓存成功注册的权威结果，标签设置中的机器号只影响标签内容，旧程序、本机参数和旧身份文件均不能改写云端机器身份。
- 后台监控不再提供或依赖自动化开关，也不周期轮询Supabase任务。它保留带工厂密钥HMAC验证的局域网UDP监听及一条不查询数据库的Supabase Realtime空闲连接；发送端建立探测、生产任务或打印机控制后先发UDP并等待签名ACK，同网段成功即不产生云通知，只有ACK超时才向目标机的密钥派生频道发送签名云通知。目标监控收到任一通道通知后仅领取本次请求；重复通知由云端原子领取和本地执行账本共同去重。安装脚本为专用UDP端口创建仅限Domain/Private与LocalSubnet的入站防火墙规则。
- 后台每2秒只在本地读取PrintExp；仅任务、打印/暂停/清洗/完成状态或整数进度等签名发生变化时覆盖Supabase当前行，ETA连续变化和单纯时间经过不上传。机器可用性以当次请求回执为准，不以人工开关推断；探测也会把现场事实更新到当前行。没有远程请求且本地状态无变化时保持零云调用，状态页只在用户打开或手动刷新时读取最近回执。
- 发送生产任务不再依据数据库中的旧在线状态直接放行。用户选择目标机后先建立一次性探测，通过UDP优先、Realtime后备的按需通知让指定机器领取；目标机必须在12秒内以同一机器ID和M1–M11机器号回传AutomaticPrint版本及现场读取的PrintExp状态。只有PrintExp在线且回应完全匹配才显示最终发送确认；发送流程没有自动化开关步骤，超时、机器号不一致或PrintExp离线仍不会创建生产任务。“发送到所有可应答机器（测试）”并行探测所有无冲突机器号，最终确认后仅向当次正确应答的机器逐台建立同一批次任务，每台结果分开显示。机器状态页另有“Realtime 全机信号测试”，强制绕过UDP并通过Supabase Realtime Broadcast并发探测全部已登记机器，用于验证跨网络通知链路。探测记录不进入任务队列展示，也不启动任何周期轮询。机器状态表、打印控制、可用性和生产任务目标选择器都以本地机器UUID匹配记录，将当前电脑显示为“M1–M11（本机）”，不靠名称猜测。
- “机器与软件”页按M1–M11一行对应一个节点：左侧固定展示PrintExp机器状态、当前PRN、进度和剩余时间；右侧展示AutomaticPrint在线状态、版本及远程任务。软件列、Realtime测试、远程更新、软件唤起和紧凑版本工具仅在开发者模式出现，普通生产用户不接触版本信息。点击一次“发布最新版本更新指令”会在后台加载最新版本后自动继续，统一选择所有版本落后或同版本但提交、协议、功能不一致的节点；批量下发前仍需确认。版本工具从`origin/main`只读获取发布历史，允许选择不早于0.1.393的目标版本并对全部待切换软件确认下发；等待、领取、切换、失败、源码完成待重启和最终版本回报都回到同一页面。升级与回滚复用同一`source_update`命令和UDP优先、Realtime后备的按需唤醒；目标机重新核对官方仓库、main线性历史及提交中的版本元数据，有已跟踪本地修改时仍拒绝覆盖。向前只快进，回滚使用保留本地内容的`reset --keep`且不脱离main，因此回滚后仍可再次升级；依赖完成后先回传结果，再由脱离任务Job的隐藏辅助进程只结束旧Python监控进程并重新运行计划任务，避免`/End`计划任务时把重启辅助程序一同终止。只有存在活动切换时每5秒读取回执，全部结束后停止。监控启动时只补领一次普通命令；AutomaticPrint有生产任务时安全重启守卫等待空闲，不强制停止。早于支持`source_update`的旧版电脑仍需最后手动更新一次。
- 新升级协议把指令协议号和功能清单写入每个发布提交的版本元数据。目标机安全重启并回报目标版本后，控制端依据已完成的源码更新回执自动通过Realtime发起一次功能探测；该流程不依赖开发者模式、手动全机测试或手动刷新，本机也与其他节点采用同一规则。只有版本、完整提交号、协议号和功能集全部一致才显示“功能已确认”；远程生产任务的现场检测还必须确认目标声明`download_layout`能力。

- 平台读取、下载和远程批次任务默认连接AutomaticPrint管理的Playwright Chrome专用配置目录；未启动时自动启动，需要登录时自动显示目标页。这是软件内的默认平台浏览器，不篡改Windows系统默认浏览器。生产批次侧栏入口缺失时直接导航到提供商的生产批次地址，不再单纯等待侧栏链接。
- Playwright Chrome在每台电脑上由调试端口和跨进程启动锁共同保证单实例；同时到达的界面与远程任务等待并复用第一个启动实例，不重复创建浏览器。生产平台工作台顶部提供通用“打开 Playwright 浏览器（提前登录）”按钮，只负责启动或显示共享浏览器，不绑定当前平台；用户可自行打开并提前登录所需网站。

- 隆丰、莆田和Haloo下载工作区各自提供状态9“已完成”来源与“自动化生成计划”按钮，后台读取本平台当前范围内状态9的生产项并自动勾选未补单的候选分组，不提交批次；仍可手动读取分类和修改勾选。页面直接说明各平台已确认的默认策略，并提供订单组成、物流、单双面、颜色、尺码档和底款六个可组合分组项；每个平台独立保存用户选择，规则变化立即清除旧预览。隆丰仍只纳入A00默认工艺，Haloo仍按黑色、白色、其他混色及黑白两档尺码解释相应选项。勾选时与最终确认窗口逐组展示实际采用的分组字段和件数，确认后才提交。页面还展示样本范围、来源批次及未纳入数量；提交前用分组中保存的同一策略重新精确读取整单，拦截已补单、过期或不完整分组。批次规则前端哈希变化由生成批次入口的实际导入关系动态定位；暂时读取失败时仍保留计划预览并禁用生成。`batch_ui/task/reads.py`复用现有工作线程并按平台路由；`local/scanning.py`后台读取目录并按来源范围丢弃旧结果。批次排版复用界面补距开关及数值，不再强制40毫米。

- ERP批次页允许等待隐藏微前端iframe挂载，再由批次入口验证实际表格。`automation/batches/local.py`统一发现本地批次；同批次号的嵌套解压目录保留外层一次，包含其全部图片，避免重复排版；预览输出目录不参与来源发现。S2B 解压根目录可包含用途、数量、时间戳和随机后缀，程序从下划线分段中还原真实的 12 位字母数字批次号，供本地列表、批次类型保存、排版输出与 PRN 继续使用。
- 蜂鸟ERP已完成候选批次由`automation/batches/supplements/completed.py`共用安全核验，由`grouping/strategy.py`按平台决定分组字段。已有批次的完成订单走补单批次接口，不改变订单完成状态；提交前通过`order_id`精确读取整单并重新核对状态、来源批次、数量和项目集合。普通精确选择接口会排除已有批次项目。平台把一次提交拆成多个批次号时，程序逐订单确认每单只归属一个新增批次并返回全部编号。Haloo和隆丰规则已用历史真实批次回放验证；莆田仍需真实平台回归。
...
- RIIN“导入图像设置”不再直接沿用机器历史参数：每组导入先唯一识别原尺寸选项、选择并回读确认；无法识别或回读时保留对话框等待用户手动选择原尺寸。管理员任务通过进行中报告把该提示及后续导入、文件输出和PrintExp步骤实时传回平台界面。
...
- “自动生成打印文件”已移入开发者“生产平台下载”工作区。`batch_ui/task/automatic_print.py`在明确的
  ...
  只接收本次最终PNG并按所选批次逐一生成独立PRN；PRN按“平台_可读批次名_件数件”命名，S2B优先使用人工批次标注、空标注回退原始编号，同批拆分时分别统计各PNG分组的实际件数；首页本地排版不再保留第二个文件夹选择入口。
  ...
  停止请求在同一批次的分区之间生效；RIIN精确任务停止、消失或提交后两分钟仍未出现时，文件任务不再等待两小时，保留已有PNG和未确认PRN并显示原因。
...
- 生产平台下载下每个蜂鸟ERP工作区提供独立“批次生成”页：已接单入口集中显示物流/订单组成候选，隆丰另显示默认路线 A00 / 多项多件和其他工艺路线的订单、项目、件数；生产中入口默认读取状态5，并可切换到保留的状态9已完成补单来源。`automation/batches/supplements/source.py`负责读取来源快照并预先核对整单及关键字段，`completed.py`让后两种来源共用分组策略；异常组在界面禁用，提交前仍复核并使用补单接口。`automation/batches/received/default_multi.py`从ERP接口预览精确项目及件数，后台用按筛选生成接口一次提交，再从生产项与批次管理核对数量、编号及整单归属。筛选为空时生成按钮禁用；生成前不预测平台拆批或批次号。
...
- 隆丰、Haloo和莆田的“批次生成”页顶部共用“打开 Playwright 浏览器”按钮；点击后立即启动或显示对应蜂鸟ERP页签，用户可先完成登录再读取预览。蜂鸟浏览器会话在打开目标页后持续监听数据区与异步登录跳转；首次打开稍后才跳到登录页时，当前后台任务继续等待用户完成登录，随后在同一次任务中重新进入原目标页并等待数据区，不再要求第二次登录或重新点击读取。
- 平台后台任务按当前动作显示停止状态；读取批次、刷新平台状态和只读预览不会显示RIIN文案。停止请求会进入Chrome启动、登录监听、生产批次表格等待和分段读取循环，Worker收到取消后结束线程并恢复界面操作。
- 打印机总览的“后台处理步骤”直接显示目标机回传的当前生产阶段、进度文字、平台、批次和件数；存在已领取或执行中的远程任务时每5秒刷新，完整文字保留在悬停提示中，任务结束后停止刷新。
- 平台自动化状态栏统一显示步骤编号、当前动作、本步骤耗时和任务总耗时，每秒更新耗时；Chrome连接、页签复用、登录等待、数据区、接口读取、记录解析、下载、解压、排版及RIIN提交分别上报状态，全部步骤同时保留在界面日志中，长文字自动换行。
- 蜂鸟ERP原始批次行到中立`BatchRecord`的转换集中在`automation/api/erp/records.py`，浏览器模块只负责页面与请求流程。
- “生产平台下载”作为跨部门共享页签，DTF与UV普通用户均可直接显示和使用。平台入口使用单选下拉菜单，一次只显示一个平台工作区；未选择时显示明确引导，切换后保留各平台已读取的批次、下载进度和日志。生产批次页不提供独立下载或全选入口；蜂鸟平台的显式“下载、排版并生成打印文件”复用当前参数继续处理，也可把所选批次发送给指定在线机器完成下载、排版和PRN。隆丰、莆田和Haloo复用蜂鸟ERP通路；S2B优先通过Supabase受限网关读取平台批次、人员标签及标准化图片数/件数，用户选择后自动触发缺失的生产图导出、轮询、下载并安全解压到`S2B/ARCHIVES`和`S2B/BATCHES`，成功解压后才标记下载；原始S2B Token只在服务端解密，网关不可用时才回退专用浏览器登录。浏览器登录成功后会把Token交给同一受限网关验证，并在服务端AES-GCM加密更新`S2B:DTF`凭据，客户端日志和配置不保存原始Token。S2B工作区的“批次预览”页通过只读网关分页读取全部`status=1`待排产项，按订单号保持整单，使用客户保存的组合显示模拟批次、订单/项目/件数和订单号；规则变化后旧预览清空。站点的建批接口已确认是`POST /req/factory/orderProductBatchNumber`，请求包含计划名称、人员、超期小时、备注、生产项ID集合和是否补单；应用仍未调用该写接口。亿点万象只下载具体勾选批次中的新增稿件，与UV排版分开。下载和自动打印动作都不自动创建生产批次。
- 生产平台的目标机器选择由`batch_ui/platform/remote/dialog.py`与`remote/queue.py`合并状态心跳和
  `machine_commands`队列：逐机显示当前PrintExp任务、后台处理中任务、最早等待任务和队列总数；发送的
  批次把平台返回的项目数、件数及平台原始批次号写入命令载荷；蜂鸟12位数字和S2B 12位大写字母数字
  分别按平台契约校验，历史命令没有数量时显示未知而不猜测。服务端列表始终返回
  全部活动命令并附最近完成记录，避免实时控制记录挤掉较早的等待下载任务。
- 主窗口滚动区外的唯一固定菜单集中版本、检查更新、打印参数、DTF平台账号和开发者入口；排版历史、测试与诊断及完整测试随开发者模式在同一区域显示，不再保留本地排版底部菜单。后台软件更新不创建状态标签、进度条或生产任务记录。
- 固定菜单中的“DTF 平台账号”窗口只读取 Supabase 账号配置状态，不下发原始凭据。`dtf-platform-auth`保留 JWT 校验并另验工厂共享密钥，服务端 Secret 集中存放 Haloo、隆丰、莆田、一朵云、七创、汉森、方果、SDS1、SDS2 九个平台的现有私有配置；S2B由既有独立网关管理，赛博已退出合作。`scripts/sync_dtf_platform_credentials.py`用于管理员从 after-sales 私有配置重复同步；网页令牌配置不代表登录仍有效。当前服务端登录探针验证一朵云、七创、汉森、SDS1、SDS2成功，方果用原 after-sales 登录代码也返回账号密码错误，待更新私有配置后重新同步。账号状态不等于批次下载已实现；下载页目前仍只有原先已接入的平台。
...
- 结构测试检查各业务子目录最多五个直接实现模块，并锁定批次、RIIN和膜补距的唯一归属；已存在的少量超过200行模块按当前行数冻结上限，后续涉及其职责时优先抽取且不得继续增长。`ui/`根目录是跨功能界面命名空间，其具体业务子目录逐一检查，不能把这一根目录误当单个组件拆分。
