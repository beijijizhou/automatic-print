...
- Supabase机器状态与远程任务中心由`202609220001/2`状态迁移及`202609230001/2/3`指令迁移维护；表仅service role可访问，Edge Function通过受限工厂密钥提供状态、下载任务和实时控制信号接口。`automation/api/printerexp/`每2秒从正在运行的PrintExp进程反查真实安装目录，读取保留小数的实际进度并按变化或60秒心跳上报，小数变化直接参与剩余时间估算；显示名只接受同一台AutomaticPrint排版设置中的M1–M11机器号，旧环境变量、电脑名和其他备注不得覆盖。主工作台固定显示M1到M11并每10秒后台刷新，忽略旧名称记录；同一机器号存在多个在线电脑时合并为一个冲突机位并禁止远程控制，避免控制错机。下载排版仍使用普通任务领取器；暂停和清洗使用独立的0.5秒高优先级控制通道，不等待下载任务完成，新的同机控制信号会替代尚未执行的旧信号。PrintExp按钮通过直接窗口点击消息执行，不移动鼠标或依赖坐标。“暂停打印”只切换到已暂停；“清洗后自动启动”会先确认当前正在打印并自动暂停，确认暂停后才启动清洗，等待清洗状态结束后再恢复打印，任一步状态不明或15分钟超时均不继续下发下一步并回传原因。源码安装/更新脚本会以管理员权限注册最高权限登录计划任务，确保监控与PrintExp权限一致；主程序启动时先结束仍占用单实例锁的旧版监控，再优先重启计划任务并移除旧的普通权限启动项，没有安装计划任务时只降级保持状态同步。过程和结果统一回传中心。
