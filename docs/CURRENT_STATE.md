...
- “生产平台下载”作为主工作台独立页签，仅随开发者模式显示；支持多选已配置平台，每个平台独立显示批次、下载进度和日志。隆丰、莆田和Haloo复用蜂鸟ERP通路；S2B复用专用浏览器登录，从导出记录读取`download_url`，下载到`S2B/ARCHIVES`并安全解压到`S2B/BATCHES`。当前只读取已生成批次并下载、解压，不触发排版，也不开放自动创建生产批次。
