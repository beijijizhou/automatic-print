...
- 主界面顶部依次为版本/打印参数/开发者菜单和“当前生产信息”；后者显示机器、平台、膜规格、有效画布及同一个本地`cutter.mode`。生产模式是用户唯一可见的模式入口，批次膜规格卡片与切膜参数页不再重复显示模式下拉框。主界面移除手动“检查更新”按钮；启动时后台静默检查，不向生产任务进度写入“软件更新／正在检查更新”，只有显式或实际应用更新时才使用活动记录。
...
- 打印机状态页提供“一键开启全部自动化／一键关闭全部自动化”。关闭信号通过带工厂密钥HMAC签名的局域网UDP广播发送给M1–M11；各后台监控持久进入零云流量状态，停止Supabase状态上报、下载任务领取和打印机控制轮询，状态页也停止自动刷新，但电脑、AutomaticPrint、PrintExp和本地UDP监听保持运行。开启同样通过局域网签名广播立即恢复，无需停用期间轮询Supabase；安装脚本为专用UDP端口创建仅限Domain/Private与LocalSubnet的入站防火墙规则。
