...
- 打印机状态页提供每台机器都可使用的“打印历史”子页。选择M1–M11后只在用户点击时通知目标节点，目标节点直接读取自己的PrintExp每日主日志，以“启动任务/作业打印完成”配对取得可靠开始、结束时间，并用`Data/recordTask.tf`补充原始路径；默认返回本机日期的今天和昨天。若目标机没有对应日期日志，则明确标注日期降级并显示最近两个可用日志，不能把缺少当日日志误报成没有历史。历史不周期上传、不复制到状态表，也不建立云端历史库。跨机器返回仅使用现有命令信封暂存，请求端读到后立即删除整条临时命令；本机查询完全不经过Supabase。
