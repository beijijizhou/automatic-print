...
- 打印机状态页的“版本管理”按M1–M11独立显示当前版本、所选目标版本及等待、领取、切换、失败、源码完成待重启和最终版本回报，本机按UUID明确标为“（本机）”。版本列表在后台从`origin/main`读取；用户可选择已发布且不早于0.1.393的历史版本，全选待切换机器、取消全选或逐机勾选，并在下发前查看每台当前→目标版本及完整提交号。升级与回滚复用同一`source_update`命令和签名UDP立即唤醒；目标机重新核对官方仓库、main线性历史及提交中的版本元数据，有已跟踪本地修改时拒绝覆盖。向前只快进，回滚使用保留本地内容的`reset --keep`且不脱离main，因此回滚后仍可再次升级；依赖完成后先回传结果，再安全重启。只有存在活动切换时每5秒读取回执，全部结束后停止。监控启动时只补领一次普通命令；AutomaticPrint有生产任务时安全重启守卫等待空闲，不强制停止。早于支持`source_update`的旧版电脑仍需最后手动更新一次。
