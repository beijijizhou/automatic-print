...
- 隆丰“已接单生成批次”页新增默认工艺路线 A00 / 多项多件专用入口：`automation/batches/default_multi.py`从ERP接口预览精确项目及件数，`batch_ui/platform/routes.py`显示读取和直接生成按钮，后台用按筛选生成接口一次提交，再从生产项与批次管理核对数量、编号及整单归属。若当前筛选为空，按钮禁用。其他工艺路线入口仍可用。
