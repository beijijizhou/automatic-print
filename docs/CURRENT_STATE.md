...
- 后台监控不再提供或依赖自动化开关，也不周期轮询Supabase任务。它平时保留带工厂密钥HMAC验证的局域网UDP监听；发送端建立探测、生产任务或打印机控制后，另发只针对目标机器ID的签名通知，目标监控仅领取本次请求。安装脚本为专用UDP端口创建仅限Domain/Private与LocalSubnet的入站防火墙规则。
...
- 发送生产任务不再依据数据库中的旧在线状态直接放行。用户选择目标机后先建立一次性探测并通过签名局域网通知指定机器领取；目标机必须在12秒内以同一机器ID和M1–M11机器号回传AutomaticPrint版本及现场读取的PrintExp状态。只有PrintExp在线且回应完全匹配才显示最终发送确认；发送流程没有自动化开关步骤，超时、机器号不一致或PrintExp离线仍不会创建生产任务。“发送到所有可应答机器（测试）”并行探测所有无冲突机器号，最终确认后仅向当次正确应答的机器逐台建立同一批次任务，每台结果分开显示。探测记录不进入任务队列展示，也不启动任何周期轮询。机器状态表、打印控制、可用性和生产任务目标选择器都以本地机器UUID匹配记录，将当前电脑显示为“M1–M11（本机）”，不靠名称猜测。
- 打印机状态页的“版本管理”按M1–M11独立显示当前版本、所选目标版本及等待、领取、切换、失败、源码完成待重启和最终版本回报，本机按UUID明确标为“（本机）”。版本列表在后台从`origin/main`读取；用户可选择已发布且不早于0.1.393的历史版本，全选待切换机器、取消全选或逐机勾选，并在下发前查看每台当前→目标版本及完整提交号。升级与回滚复用同一`source_update`命令和签名UDP立即唤醒；目标机重新核对官方仓库、main线性历史及提交中的版本元数据，有已跟踪本地修改时拒绝覆盖。向前只快进，回滚使用保留本地内容的`reset --keep`且不脱离main，因此回滚后仍可再次升级；依赖完成后先回传结果，再由脱离监控任务Job的隐藏辅助进程结束旧监控并重新运行计划任务，避免辅助进程随旧任务一同被终止。只有存在活动切换时每5秒读取回执，全部结束后停止。监控启动时只补领一次普通命令；AutomaticPrint有生产任务时安全重启守卫等待空闲，不强制停止。早于支持`source_update`的旧版电脑仍需最后手动更新一次。
