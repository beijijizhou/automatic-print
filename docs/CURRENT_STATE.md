...
- Supabase机器状态与远程任务中心由状态、指令和事件可用性迁移维护；表仅service role可访问，Edge Function通过受限工厂密钥提供状态、下载任务和实时控制信号接口。`automation/api/printerexp/`每2秒在本机读取PrintExp，只有任务、打印机状态或整数进度变化才覆盖当前行；发送端建立请求后先用HMAC签名UDP通知唯一机器ID并等待目标机签名ACK，仅在ACK超时时用Supabase Realtime跨网络通知，不做周期心跳、命令轮询或界面定时刷新。后台机器号只读取`machine-slot-v2`，普通参数保存、旧环境变量、电脑名、注册表、`machine-name`和旧`machine-binding`均不能更新；只有用户明确操作机器号下拉框才调用独立绑定函数。主工作台固定显示M1到M11，同号电脑标记冲突并禁止控制。生产批次页把勾选批次与排版参数发给实时探测通过的机器，目标机连续执行下载、排版、生成PRN和加载PrintExp，不启动物理打印。暂停和清洗继续使用独立高优先级控制通道；清洗逻辑保留现状，后续单独完成现场验证后再修改。源码安装/更新脚本注册与PrintExp同权限的登录计划任务，更新时重启监控。过程和结果统一回传中心。
...
- 后台机器号使用旧程序未知的`machine-slot-v2`文件并保持原机器UUID。普通参数自动保存不再调用身份绑定；只有用户明确操作M1–M11下拉框才调用`bind_machine_slot()`。旧版本窗口即使持续把`machine-name`和`machine-binding`写成其他号码，也不能覆盖后台监控与远程命令身份；没有v2绑定的电脑显示“未设置机器号”，不得从旧设置自动迁移或猜测。
