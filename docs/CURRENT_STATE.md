...
- Supabase机器状态与远程任务中心由`202609220001/2`状态迁移、`202609230001/2/3/4/5`指令迁移及`202609240001`事件可用性迁移维护；表仅service role可访问，Edge Function通过受限工厂密钥提供状态、下载任务和实时控制信号接口。`automation/api/printerexp/`每2秒从正在运行的PrintExp进程反查真实安装目录，读取保留小数的实际进度，并只在进程启动或状态变化时上报，不发送周期心跳，小数变化直接参与剩余时间估算；显示名只接受同一台AutomaticPrint排版设置中的M1–M11机器号，并同步到当前用户本地配置文件供普通界面与管理员计划任务一致读取，旧环境变量、电脑名和其他备注不得覆盖。偏好设置恢复和程序性下拉框同步仅读取机器号，只有用户明确操作机器号下拉框或保存参数才会更新共享机器身份，测试或预览窗口的隔离配置不得覆盖生产机器号。主工作台固定显示M1到M11并每10秒后台刷新，忽略旧名称记录；同一机器号存在多个在线电脑时合并为一个冲突机位并禁止远程控制，避免控制错机。生产批次页的“发送到指定机器生成 PRN”直接传递当前勾选批次与排版参数，后台读取无冲突的在线机器供用户选择；目标机领取后从下载、排版连续执行到生成PRN并加载PrintExp，不启动物理打印，Haloo、隆丰、莆田和S2B均由服务端放行。暂停和清洗使用独立的0.5秒高优先级控制通道，不等待下载任务完成，新的同机控制信号会替代尚未执行的旧信号。PrintExp自绘按钮通过发送其真实`WM_COMMAND`通知执行，不移动鼠标或依赖坐标；普通鼠标消息和`BM_CLICK`在活动批次中可能被忽略。“暂停打印”只切换到已暂停；“清洗后自动启动”先把PrintExp当前用户参数明确设置并回读为8个喷头全部清洗、强度中，再确认当前正在打印并自动暂停。PrintExp的清洗按钮处理会同步阻塞至设备动作完成，因此控制器在独立线程发送命令并以处理返回作为完成确认，不要求事后才能读取到的短暂“正在清洗”文字；命令完成后若PrintExp未自行恢复才发送继续打印，任一步状态不明或15分钟超时均不继续下发下一步并回传原因。源码安装/更新脚本会以管理员权限注册最高权限登录计划任务，确保监控与PrintExp权限一致；主程序启动时先结束仍占用单实例锁的旧版监控，再优先重启计划任务并移除旧的普通权限启动项，没有安装计划任务时只降级保持状态同步。过程和结果统一回传中心。
...
- 打印机状态页提供“测试机器信号”按钮，并发探测已登记且无编号冲突的M1–M11，分别显示本次实时响应、上次上报版本、当前版本数量、待更新和超时机器；检测结果留在状态页，不进入生产任务中心。该页同时提供“一键开启全部自动化／一键关闭全部自动化”。关闭信号通过带工厂密钥HMAC签名的局域网UDP广播发送给M1–M11；各后台监控持久进入零云流量状态，停止Supabase状态上报、下载任务领取和打印机控制轮询，状态页也停止自动刷新，但电脑、AutomaticPrint、PrintExp和本地UDP监听保持运行。开启同样通过局域网签名广播立即恢复，无需停用期间轮询Supabase；安装脚本为专用UDP端口创建仅限Domain/Private与LocalSubnet的入站防火墙规则。
- 云调用安全门以“未明确开启即关闭”为默认，新安装和更新后不会自动连接Supabase。用户开启后，机器启动与任务、状态或整数进度变化才上报，ETA连续变化和时间经过都不触发请求。机器默认可用；任务下发20秒仍未被领取且期间没有任何反馈时，自动标记为不可用，后续真实反馈自动恢复。状态页可共享手工设置“自动判断 / 可用 / 不可用”，并每60秒读取一次。
- 发送生产任务不再依据数据库中的旧在线状态直接放行。用户选择目标机后先通过高优先级控制通道发送一次性探测；目标机必须在12秒内以同一机器ID和M1–M11机器号回传AutomaticPrint版本、自动化开关及现场读取的PrintExp状态。只有PrintExp在线且回应完全匹配才显示最终发送确认；超时、机器号不一致、自动化关闭或PrintExp离线都不会创建生产任务。探测记录不进入任务队列展示，也不恢复周期心跳。
...
- 蜂鸟浏览器会话在打开隆丰、Haloo或莆田目标页后持续监听数据区与异步登录跳转；首次打开稍后才跳到登录页时，当前后台任务继续等待用户完成登录，随后在同一次任务中重新进入原目标页并等待数据区，不再要求第二次登录或重新点击读取。
