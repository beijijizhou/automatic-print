...
- 隆丰 A00 已接单多件订单由`automation/batches/received_sizes.py`读取接口快照并规划：跨尺码多项多件整单优先，同尺码的单项多件与多项多件再按尺码分别生成。复用`automation/api/erp/items.py`的整单查询及精确选择生成接口，每次不超过200生产项且只在订单边界分段；逐单确认入批次号，单组不确定不重试，其余独立组继续。当前通过开发操作调用，单项单件留待其他批次流程。
