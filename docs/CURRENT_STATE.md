...
- 隆丰 A05-无印花的生产中补单由`automation/batches/supplement_sizes.py`通过已登录会话的ERP接口读取、按尺码制定计划并提交；颜色不参与分组，跨尺码订单单独成组。接口可能按来源批次拆成多个补单批次，提交后按每项新增编号核对，不自动重试。当前脚本入口由开发操作调用，尚未装配为可见按钮。
