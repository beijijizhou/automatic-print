...
- 主界面顶部依次为版本/打印参数/开发者菜单和“当前生产信息”；后者显示机器、平台、膜规格、有效画布及同一个本地`cutter.mode`。生产模式是用户唯一可见的模式入口，批次膜规格卡片与切膜参数页不再重复显示模式下拉框。主界面移除手动“检查更新”按钮；软件检查、下载、应用和重启全部留在更新模块内部，不写入只用于生产步骤的任务中心。
...
- 主窗口滚动区外的唯一固定菜单集中版本、打印参数、DTF平台账号和开发者入口；排版历史、测试与诊断及完整测试随开发者模式在同一区域显示，不再保留本地排版底部菜单。后台软件更新不创建状态标签、进度条或生产任务记录。
