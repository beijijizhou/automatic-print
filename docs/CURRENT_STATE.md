...
- Supabase机器状态与远程任务中心由`202609220001/2`状态迁移及`202609230001/2/3`指令迁移维护；表仅service role可访问，Edge Function通过受限工厂密钥提供状态、下载任务和实时控制信号接口。`automation/api/printerexp/`每2秒从正在运行的PrintExp进程反查真实安装目录，读取保留小数的实际进度并按变化或60秒心跳上报，小数变化直接参与剩余时间估算；显示名只接受同一台AutomaticPrint排版设置中的M1–M11机器号，并同步到当前用户本地配置文件供普通界面与管理员计划任务一致读取，旧环境变量、电脑名和其他备注不得覆盖。偏好设置恢复仅读取机器号，只有用户明确选择或保存才会更新共享机器身份，测试或预览窗口的隔离配置不得覆盖生产机器号。主工作台固定显示M1到M11并每10秒后台刷新，忽略旧名称记录；同一机器号存在多个在线电脑时合并为一个冲突机位并禁止远程控制，避免控制错机。下载排版仍使用普通任务领取器；暂停和清洗使用独立的0.5秒高优先级控制通道，不等待下载任务完成，新的同机控制信号会替代尚未执行的旧信号。PrintExp自绘按钮通过发送其真实`WM_COMMAND`通知执行，不移动鼠标或依赖坐标；普通鼠标消息和`BM_CLICK`在活动批次中可能被忽略。“暂停打印”只切换到已暂停；“清洗后自动启动”先把PrintExp当前用户参数明确设置并回读为8个喷头全部清洗、强度中，再确认当前正在打印并自动暂停。PrintExp的清洗按钮处理会同步阻塞至设备动作完成，因此控制器在独立线程发送命令并以处理返回作为完成确认，不要求事后才能读取到的短暂“正在清洗”文字；命令完成后若PrintExp未自行恢复才发送继续打印，任一步状态不明或15分钟超时均不继续下发下一步并回传原因。源码安装/更新脚本会以管理员权限注册最高权限登录计划任务，确保监控与PrintExp权限一致；主程序启动时先结束仍占用单实例锁的旧版监控，再优先重启计划任务并移除旧的普通权限启动项，没有安装计划任务时只降级保持状态同步。过程和结果统一回传中心。
