...
- PrintExp在文件打开对话框关闭且原生控件进入待打印后，会把本次PRN写入本机装载回执；当`PrintInfo.ini`仍停留在上一任务时，监控优先显示该新PRN为0%“待打印（待复核）”。只有界面或状态文件独立确认文件名后才允许“开始打印”，装载回执本身不能触发物理打印。远程PRN结果包含生成或装载错误时，整条命令标记失败并保留详细结果，不再显示成功。
