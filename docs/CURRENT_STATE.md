...
- 发送生产任务不再依据数据库中的旧在线状态直接放行。用户选择目标机后先建立一次性探测并通过签名局域网通知指定机器领取；目标机必须在12秒内以同一机器ID和M1–M11机器号回传AutomaticPrint版本及现场读取的PrintExp状态。只有PrintExp在线且回应完全匹配才显示最终发送确认；发送流程没有自动化开关步骤，超时、机器号不一致或PrintExp离线仍不会创建生产任务。探测记录不进入任务队列展示，也不启动任何周期轮询。机器状态表、打印控制、可用性和生产任务目标选择器都以本地机器UUID匹配记录，将当前电脑显示为“M1–M11（本机）”，不靠名称猜测。
