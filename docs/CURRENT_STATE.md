...
- PrintExp实时控制增加`202609230004_start_print_control.sql`、`202609230005_start_print_action.sql`和“开始打印”。状态面板区分空闲、待打印、打印中、已暂停、清洗中和未确认；数据库指令约束显式允许`start_print`。同一按钮在0%待打印时开始新任务，在已暂停时显示“继续打印”并恢复当前任务。两种状态都必须已装载对应批次且任务名已独立核实。服务端先核对当前状态与批次名，目标机执行前再读取状态文件并核对PrintExp界面中的实际PRN名称，任一事实变化均拒绝发送指令。
