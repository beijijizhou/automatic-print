...
- 打印机状态页的“版本管理”集中全机Realtime信号检测、发布最新版本更新指令和逐机版本切换；机器总览不重复显示更新入口。该页按M1–M11独立显示当前版本、所选目标版本及等待、领取、切换、失败、源码完成待重启和最终版本回报，本机按UUID明确标为“（本机）”。版本列表在后台从`origin/main`只读获取，控制端有已跟踪本地修改也不阻止查看和下发；用户可选择已发布且不早于0.1.393的历史版本，全选待切换机器、取消全选或逐机勾选，并在下发前查看每台当前→目标版本及完整提交号。升级与回滚复用同一`source_update`命令和UDP优先、Realtime后备的按需唤醒；目标机重新核对官方仓库、main线性历史及提交中的版本元数据，有已跟踪本地修改时仍拒绝覆盖。向前只快进，回滚使用保留本地内容的`reset --keep`且不脱离main，因此回滚后仍可再次升级；依赖完成后先回传结果，再由脱离任务Job的隐藏辅助进程只结束旧Python监控进程并重新运行计划任务，避免`/End`计划任务时把重启辅助程序一同终止。只有存在活动切换时每5秒读取回执，全部结束后停止。监控启动时只补领一次普通命令；AutomaticPrint有生产任务时安全重启守卫等待空闲，不强制停止。早于支持`source_update`的旧版电脑仍需最后手动更新一次。
- 新升级协议把指令协议号和功能清单写入每个发布提交的版本元数据。目标机安全重启后，版本管理自动发起一次实时探测，只有版本、完整提交号、协议号和功能集全部一致才显示“功能已确认”；远程生产任务的现场检测还必须确认目标声明`download_layout`能力。
