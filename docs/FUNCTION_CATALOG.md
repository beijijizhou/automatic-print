...
| libvips 分块渲染与运行时门禁 | `layout_engine/vips_renderer.py`, `engine_info.py`, `atomic_png.py`, `save_progress.py`, `png_codecs/` | 使用浅层画布图和固定快速滤波完成合成编码；外层延迟任务通过共享门禁串行进入，libvips 内部仍可多线程。保存报告按首批数据、持续生成写入、编码收尾和原子发布的真实文件增长边界计时，并明确延迟流水线内的解码、合成、压缩与写入相互重叠。 |
