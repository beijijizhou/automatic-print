...
| 并排虚拟尺寸上限 | `layout_engine/planning/zones/pair_width.py`, `ui/cutter_settings_state.py`, `ui/header_gap.py` | 主界面与设置页同步可编辑原图宽度上限（默认310毫米）；普通贪心和隆丰共刀仅对S–XL且原宽不超过上限的图按刀位安全宽度等比缩小，双面同倍率；超限完整订单进入独立旋转区。报告原/采用尺寸，不修改源图。 |
...
| 隆丰多批共用固定刀位 | `automation/workflows/shared_knife.py`, `automation/workflows/shared_knife_batches.py`, `layout_engine/planning/columns/adaptive_knife.py`, `layout_engine/rendering/storage/plan_partition.py`, `layout_engine/planning/zones/pair_width.py`, `batch_ui/platform/pages.py`, `batch_ui/local/processing.py`, `automation/api/riin/jobs.py` | 在生产平台下载页一次读取多批，跨批次锁定多排区刀位并跳过四膜省膜比较；S–XL原宽不超过用户上限可等比缩小，超限完整订单转入独立刀位的旋转区。沿完整订单和排版行边界把不同刀位分成独立PNG。逐文件核对实际刀位、订单、膜宽及像素安全通道，固定刀位归`切膜机文件/常规`，其余刀位归`切膜机文件/旋转`；不同刀位分别生成PRN，绝不混作连续打印。任务子目录防覆盖，单行不成双排但刀位不变仍属常规，不启动实体打印。 |
