...
| 分段输出 | `layout_engine/rendering/storage/segmented_output.py`, `layout_engine/rendering/segment_result.py`, `layout_engine/rendering/storage/atomic_png.py` | 按完整行/订单切分，失败文件不可冒充可打印结果；所有使用虚拟补距的平台共用独立进程并行保存资格，不维护第二份平台名单；汇总结果由`segment_result.py`统一构造。 |
...
| 隆丰多批共用固定刀位 | `automation/workflows/shared_knife.py`, `automation/workflows/shared_knife_batches.py`, `layout_engine/planning/zones/pair_width.py`, `layout_engine/planning/rotation/whole_rotation.py`, `batch_ui/platform/pages.py`, `batch_ui/local/processing.py`, `automation/api/riin/jobs.py` | 在生产平台下载页用独立按钮一次选定并读取多批，沿用当前刀位分别生成；复用虚拟尺寸覆盖让可缩小的超宽图进入双列，双面同倍率并报告改动；严格模式禁止超宽恢复暗中改刀，固定刀位失败时保留诊断、按原策略重排刀位变化批次，逐张核对实际刀位、整批订单、膜宽及像素安全通道后分别归入`切膜机文件/常规`与`切膜机文件/旋转`，任务子目录防覆盖，单行不成双排但固定刀位不变仍属常规，PRN按归档路径导入。仍不启动实体打印。 |
