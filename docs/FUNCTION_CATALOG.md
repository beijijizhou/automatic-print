...
| 并排虚拟尺寸上限 | `layout_engine/planning/zones/pair_width.py` | 普通贪心并排只处理S–L；隆丰共刀入口按所选固定刀位两侧安全宽度处理原始打印宽度不超过310毫米的图，双面同倍率；更宽的完整订单进入非固定旋转区。按用户上限及刀码安全占位生成尺寸覆盖，实际列数由共享规划决定；报告原/采用尺寸，不修改源图。 |
...
| 隆丰多批共用固定刀位 | `automation/workflows/shared_knife.py`, `automation/workflows/shared_knife_batches.py`, `layout_engine/planning/columns/adaptive_knife.py`, `layout_engine/rendering/storage/plan_partition.py`, `layout_engine/planning/zones/pair_width.py`, `batch_ui/platform/pages.py`, `batch_ui/local/processing.py`, `automation/api/riin/jobs.py` | 在生产平台下载页一次读取多批，跨批次锁定多排区刀位并跳过四膜省膜比较；310毫米内可等比缩小，超宽完整订单转入独立刀位的旋转区。沿完整订单和排版行边界把不同刀位分成独立PNG。逐文件核对实际刀位、订单、膜宽及像素安全通道，固定刀位归`切膜机文件/常规`，其余刀位归`切膜机文件/旋转`；不同刀位分别生成PRN，绝不混作连续打印。任务子目录防覆盖，单行不成双排但刀位不变仍属常规，不启动实体打印。 |
