...
| 标签、平台文字和刀码 | `layout_engine/labels.py`, `layout_engine/text/fonts.py`, `dynamic_label.py`, `marker_stack.py`, `marker_space.py`, `platform_space.py`, `platform_label.py`, `gap_fallback.py` | 字体加载与有界线程缓存只有一个所有者；平台文字由平台名和原图尺码组成，以二维码卡片高度为字号上限；测量、预览、输出使用同一几何结果，旋转后整个平台尺码标签随二维码同向旋转，并只复用图片宽度内经像素验证的透明空位；外置刀码紧贴图片边缘。整批复用透明带失败时由共享回退改用外置标签真实占位并继续，报告原值、采用值、影响和修改入口。 |
