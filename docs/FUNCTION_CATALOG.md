...
| 膜标签透明补距 | `layout_engine/labeling/base/header_gap.py`, `layout_engine/labeling/gap/preparation.py`, `layout_engine/labeling/gap/cache_files.py` | 基础模块负责缓存、路径映射和批次编排；像素模块只测标签下方有限条带并以libvips生成无损透明补距副本，Windows非顺序读取或写入失败时使用Pillow兼容路径；缓存文件模块统一处理Windows占用重试与临时文件回收，单张仍失败时保留原图继续整批，不改原图。 |
