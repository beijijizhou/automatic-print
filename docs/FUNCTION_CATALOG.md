...
| 隆丰多批共用固定刀位 | `automation/workflows/shared_knife.py`, `automation/workflows/shared_knife_batches.py`, `layout_engine/planning/rotation/whole_rotation.py`, `batch_ui/platform/pages.py`, `batch_ui/local/processing.py`, `automation/api/riin/jobs.py` | 在生产平台下载页用独立按钮一次选定并读取多批，沿用当前刀位分别生成；严格模式禁止超宽恢复暗中改刀，固定刀位失败时保留诊断、按原策略重排需值守批次，逐张核对实际刀位、整批订单、膜宽及像素安全通道后分别归入“连续打印”候选与“需值守”，PRN按归档路径导入。仍不启动实体打印。 |
