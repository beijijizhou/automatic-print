...
| 普通行和自动多列规划 | `layout_engine/planning/base/planner.py`, `layout_engine/planning/columns/cutter_planner.py`, `layout_engine/planning/columns/dynamic_columns.py`, `layout_engine/planning/columns/column_solver.py`, `layout_engine/planning/columns/choice_cutter.py` | 膜宽与真实占位决定列数；先淘汰存在无列可放或无法填满全部列的物理无解候选，再用有记忆的列匹配求解；一至八列共用同一Placement入口。开发者紧凑排版额外计算RIIN原始顺序对照，并在现有生产优先级下比较同一固定刀位的逐件方向选择；只有RIIN对照候选允许任意相邻单图共享一行，正式优化候选仍保持颜色与完整订单边界；双面方向一致，普通模式不启用。 |
...
| 多刀位安全事实 | `layout_engine/cutting/geometry/knife_change_gap.py`, `layout_engine/cutting/validation/cut_validation.py` | 输出、预览、像素检查和报告复用实际刀位列表；每条安全通道独立核验；双排转旋转及旋转转双排会补足不足距离并压缩可安全移除的多余空白，使相邻左侧识别刀码达到设定目标，内容防重叠优先；批次结束至少保留设定距离。该规则仅在开发者模式正数参数下调用并使用独立缓存版本，普通模式保持原算法和缓存键。 |
...
| 二维码侧别与空白带 | `layout_engine/labeling/platform/qr_corners.py`, `layout_engine/labeling/markers/qr_placement.py`, `layout_engine/labeling/base/header_region.py`, `layout_engine/labeling/header_components.py` | 只测顶部有限条带的位置和可用空间，不做二维码解码或整图像素读取；连通白卡片的合并归`header_components.py`。 |
...
| 膜标签透明补距 | `layout_engine/labeling/base/header_gap.py`, `layout_engine/labeling/gap/batch.py`, `layout_engine/labeling/gap/report.py`, `layout_engine/labeling/gap/virtual.py`, `layout_engine/labeling/gap/virtual_cache.py`, `layout_engine/labeling/gap/preparation.py`, `layout_engine/labeling/gap/cache_files.py` | 基础模块负责单图缓存，`batch.py`负责并行批次编排，`report.py`负责来源复核与异常记录；Haloo、S2B、莆田与隆丰只记录持久化的透明间距几何并在最终合成时插入，不生成中间图片；其他平台保留无损补距副本兼容路径。缓存文件模块统一处理Windows占用重试与临时文件回收，单张仍失败时保留原图继续整批，不改原图。 |
...
| libvips 分块渲染与运行时门禁 | `layout_engine/rendering/engines/vips_renderer.py`, `layout_engine/rendering/engines/row_bounds.py`, `layout_engine/rendering/engine_info.py`, `layout_engine/rendering/storage/atomic_png.py`, `layout_engine/rendering/storage/save_progress.py`, `layout_engine/rendering/png/` | `row_bounds.py`按最终画布坐标保留高于排版基线的刀码；`layout_engine/rendering/png/row_stream.py` 按最终 Y 顺序逐行解码、合成、核对刀位 alpha、固定 UP 滤波、无损 RLE 压缩并写入，每个排版行只求值一次且不生成中间图片；外层任务通过共享门禁串行进入，libvips 内部仍可多线程。完整 PNG 发布后顺序检查全部数据块 CRC、RGBA 尺寸和文件结构，不再二次解压已核对的像素流。 |
| 排版公共入口与生成编排 | `layout_engine/__init__.py`, `layout_engine/pipeline/service.py`, `layout_engine/pipeline/render_output.py`, `layout_engine/domain/models.py` | 包入口只导出稳定公共能力；`service.py`编排计划与结果，`render_output.py`集中执行合成、编码和保存后验证；领域模型保持唯一所有者。 |
...
| Haloo已生产测试批次计划 | `automation/batches/completed.py`, `automation/batches/classification.py`, `automation/api/erp/items.py`, `batch_ui/platform/completed.py` | 状态9快照逐项读取实际生产图`A面/B面`，按整单、物流、订单组成、底款、黑白与尺码档生成显式ID计划；界面仅展示有限快照候选及未纳入数量，不保证跨页整单完整；接口未证明不会重开生产前，提交请求由安全门禁拒绝。 |
