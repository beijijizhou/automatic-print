...
| 输出格式选择与并行分块 BigTIFF | `layout_engine/rendering/engines/output_encoder.py`, `layout_engine/rendering/storage/atomic_tiff.py`, `ui/settings/output/format.py`, `ui/batch_input_panel.py` | 仅开发者模式可选择 TIFF；主界面输出参数与完整打印设置双向同步，关闭开发者模式恢复 PNG；复用同一 libvips 画布，按整幅宽度和保存内存预算选择256至4096行 Strip，使用 tifffile/imagecodecs 多线程独立无损压缩并由单一写入器登记偏移；最终辅助线画布编码前完成整批刀位复核，保留透明通道、DPI和原子发布。 |
