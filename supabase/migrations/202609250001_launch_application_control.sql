...
begin
  ...
  where target_machine_id = target_id
    and action in ('start_print', 'pause_print', 'clean_resume', 'probe', 'launch_app')
    and status = 'queued'
    and expires_at <= now();
  ...
  where id = (
    ...
    where target_machine_id = target_id
      ...
      and status = 'queued'
      and expires_at > now()
    order by case when action in ('probe', 'launch_app') then 0 else 1 end, created_at
