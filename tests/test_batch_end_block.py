...
def test_preview_uses_same_end_block_rectangle():
    ...
    settings=LayoutSettings(dpi=25.4,batch_end_block=True,transition_lines=False)
    block=transition_rects(planned,settings,580)[0]
...
def test_end_block_only_last_segment_and_source_pixels_preserved(tmp_path,engine):
    paths=[]
