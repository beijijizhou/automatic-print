...
def test_unknown_progress_uses_compact_spinner(tmp_path):
    window = make_window(tmp_path)
    window.started_at = window.stage_started_at = 1
    window.update_progress('保存图片', 390_000_000, 0, 'batch.png')
    APP.processEvents()
    assert window.busy_spinner.isActive()
    assert window.busy_spinner.isVisible()
...
def test_known_progress_keeps_percentage_bar(tmp_path):
    window = make_window(tmp_path)
    window.started_at = window.stage_started_at = 1
