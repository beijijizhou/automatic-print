...
def test_costs_are_developer_only_and_copyable_without_image_read(tmp_path):
    ...
    assert '12.933秒' in page.reference.toPlainText()
    assert '5.503秒' in page.reference.toPlainText()
    assert '23.864秒' in page.reference.toPlainText()
    assert '不是每次生产新增的等待步骤' in page.reference.toPlainText()
    ...
    assert '真实生产图片独立基准' in APP.clipboard().text()
    assert page.grab().save(str(tmp_path/'algorithm-costs.png'))
    page.records.setCurrentWidget(page.reference)
    assert page.reference.isVisible()
