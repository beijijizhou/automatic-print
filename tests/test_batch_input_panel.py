...
def test_input_cards_group_single_and_multiple_while_details_stay_below(tmp_path):
    ...
    assert inputs.isAncestorOf(home.preview_layout_button)
...
def test_preview_uses_current_folder_or_selects_one_without_printing(tmp_path, monkeypatch):
    ...
    owner.folder.setText(str(tmp_path))
    monkeypatch.setattr(QFileDialog, 'getExistingDirectory',
                        lambda *_: (_ for _ in ()).throw(AssertionError('Already selected')))
    owner.automation_home.preview_layout_button.click()
