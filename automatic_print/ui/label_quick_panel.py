...
class LabelQuickPanel(QWidget):
    ...
    def __init__(self, label, block, parent=None, window=None):
        ...
        self.preview_tabs.addTab(self.actual_preview_page, '排版结构 / 真实图片')
        ...
        overview = QCheckBox("显示整批轻量结构图（不读取缩略图；取消后查看当前订单真实图片）")
        ...
        self.algorithm_costs_button = QPushButton('算法诊断')
        self.algorithm_costs_button.setToolTip('查看排版步骤的复杂度和实际耗时，仅用于开发检查。')
