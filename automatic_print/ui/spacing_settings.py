...
def migrate_spacing(preferences):
    key = 'layout/vertical_spacing_8mm_v1'
    if not preferences.contains(key):
        if preferences.value('layout/spacing_mm', 5, float) == 5:
            preferences.setValue('layout/spacing_mm', 8)
