...
from ..controllers.generation_progress import (
    progress_count, progress_percent, remaining_seconds,
)
class GenerationActionsMixin:
    ...
    def generate(self, checked=False, *, preview_only=False) -> None:
        ...
        if not self.layout_generation.prepare():
            return
        ...
        self.layout_generation.start(worker, self.worker_bridge)
    ...
    def refresh_timing(self) -> None:
        ...
        remaining = remaining_seconds(
            self.current_stage, self.current_count, self.current_total, stage_elapsed)
        estimate = (
            f"本阶段预计还需 {duration_text(remaining)}"
            if remaining is not None else "正在计算…"
        )
        saving_detail = self._saving_detail()
        count = progress_count(
            self.current_stage, self.current_count, self.current_total)
        self.status.setText(
    ...
    def generation_finished(self, output: str, result: dict) -> None:
        ...
        remember_recent_output(self, output)
        show_progress(self)
        self.progress.setRange(0, 100)
        self.progress.setValue(100)
