...
def order_key(path):
    from automatic_print.automation.api.s2b.metadata.store import order_for_path
...
def _piece_sort(path):
    ...
    if len(parts) >= 5 and re.fullmatch(r'no\d+', parts[-2]):
        from automatic_print.layout_engine.intake.metadata.source_metadata import source_color, color_key
