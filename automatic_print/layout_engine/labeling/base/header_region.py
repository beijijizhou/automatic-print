...
import sqlite3
import numpy as np
from automatic_print.layout_engine.labeling.header_components import components as _components
from automatic_print.layout_engine.labeling.header_components import merge_vertical_sections as _merge_vertical_sections
    ...
    for box in sorted(candidates, key=lambda r: r[0]):
        if merged:
            ...
            if overlap >= short*.75 and box[0]-a[2] <= max(3, short*.12):
                ...
                continue
        merged.append(box)
    return _merge_vertical_sections(merged, w)
