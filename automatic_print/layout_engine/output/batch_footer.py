...
from automatic_print.layout_engine.labeling.base.labels import label_badge, format_label
...
def footer_text(planned, settings, notice):
    ...
    lines = [
        f"{notice} · {'+'.join(folders)} · {order_quantity(paths, scope)} · {len(paths)}张",
        f"平台：{settings.platform_name or '未设置'} · 机器：{settings.machine_number} · 标签：{label}",
    ]
    zones = {}
    for path, placement in planned:
        zones.setdefault(placement.cut_zone or '常规区', []).append((path, placement))
    for name, members in zones.items():
        rows = {}
        for _, placement in members:
            rows.setdefault(placement.row_y_px, []).append(placement)
        counts = {}
        for row in rows.values():
            if len(row) > 1:
                counts[len(row)] = counts.get(len(row), 0) + 1
        parallel = '、'.join(
            f'{columns}排{count}行' for columns, count in sorted(counts.items())) or '无并排'
        placement = members[0][1]
        knives = placement.cut_knife_xs_px or (
            (placement.cut_knife_x_px,) if placement.cut_knife_x_px is not None else ())
        ...
        length = (
            max(p.row_y_px+p.footprint_height_px for _, p in members)
            - min(p.row_y_px for _, p in members)
        )*25.4/settings.dpi/1000
